  ...
  namespace common
  {
    ...
    (std::vector<double>& _angles, const std::vector<int16_t> _strokes)
    {
      ...
      float left_wrist_roll_stroke =
        (scale * _strokes[18] + scale * _strokes[19]) * 0.5;
      float right_wrist_roll_stroke =
        (scale * _strokes[8] + scale * _strokes[9]) * 0.5;
      float waist_pitch_stroke =
        (scale * _strokes[23] + scale * _strokes[24]) * 0.5;
      float neck_pitch_stroke =
        (scale * _strokes[1] + scale * _strokes[2]) * 0.5;
      float deg2Rad = M_PI / 180.0;
      float knee_angle  = - deg2Rad * LegInvTable(scale * _strokes[27]);
      float ankle_angle = deg2Rad * LegInvTable(scale * _strokes[26]);

      // can_order -> ros_order
      _angles[0] =
        deg2Rad * scale * _strokes[25];
      _angles[1] =
        deg2Rad * WaistPitchInvTable(waist_pitch_stroke);
      _angles[2] =
        deg2Rad * WaistRollInvTable(scale * _strokes[23] - waist_pitch_stroke);

      _angles[3] =
        -deg2Rad * ShoulderPitchInvTable(scale * _strokes[13]);
      _angles[4] =
        deg2Rad * ShoulderRollInvTable(scale * _strokes[14]);
      _angles[5] =
        -deg2Rad * scale * _strokes[15];
      _angles[6] =
        -deg2Rad * ElbowPitchInvTable(scale * _strokes[16]);
      _angles[7] =
        -deg2Rad * scale * _strokes[17];
      _angles[8] =
        deg2Rad * WristPitchInvTable(scale * _strokes[18] - left_wrist_roll_stroke);
      _angles[9] =
        -deg2Rad * WristRollInvTable(left_wrist_roll_stroke);
      _angles[10] =
        -deg2Rad * scale * _strokes[21];
      _angles[11] =
        deg2Rad * (scale * _strokes[20] * 8.333 + 65.0);
      _angles[12] = 0;
      _angles[13] = 0;
      _angles[14] =
        -deg2Rad * (scale * _strokes[22] * 11.111 + 15.0);
      ...
      _angles[25] =
        -deg2Rad * scale * _strokes[11];
      _angles[26] =
        -deg2Rad * (scale * _strokes[10] * 8.333 + 65.0);
      _angles[27] = 0;
      _angles[28] = 0;
      _angles[29] =
        deg2Rad * (scale * _strokes[12] * 5.556 + 15.0);
