/*
  ...
  @define ElbowPitchTable from elbow-p offset 0
...
namespace aero
{
  namespace common
  {
    ...
    (std::vector<int16_t>& _strokes, const std::vector<double> _angles)
    {
      ...
      float scale = 100.0;
      dualJoint right_wrist = WristRollPitchTable(rad2Deg * r_wrist_p_joint, rad2Deg * r_wrist_r_joint);

      // ros_order -> can_order
      meta = scale * rad2Deg * r_shoulder_y_joint; //0
      meta = scale * ShoulderPitchTable(rad2Deg * r_shoulder_p_joint); //1
      ...
      meta = -scale * rad2Deg * r_hand_y_joint;//6      
      meta = -scale * (rad2Deg * r_thumb_joint - 50.0) * 0.18;//7
