...
(ros::NodeHandle &nh_param) {
  ...
  nh_param.param<int>("/teleop_joy/switch_joint_mode", switch_mode, 0);
  ...
  nh_param.getParam("/teleop_joy/ik", ik_map);
  nh_param.getParam("/teleop_joy/scale_ik", scale_ik_map);
...
void aero::teleop::ps_teleop::loop() {
  ...
  if(enable_ik) {
    ...
    robot->getEndEffectorCoords(aero::arm::rarm, aero::eef::grasp, eef);
    goal_pos.x() = eef.translation().x() + pos_dp.x();
    goal_pos.y() = eef.translation().y() + pos_dp.y();
    goal_pos.z() = eef.translation().z() + pos_dp.z();
    robot->setRobotStateToCurrentState();
    ROS_INFO("x:%f y:%f z:%f", goal_pos.x(), goal_pos.y(), goal_pos.z());

    aero::Transform pose = goal_pos * init_rot * rot_top;
    aero::Quaternion qq(pose.linear());
    ROS_INFO("%f,%f",qq.w(), qq.x());  
    if(robot->setFromIK(aero::arm::rarm, aero::ikrange::arm_lifter, pose, aero::eef::grasp)){
      ik_bool_msg.data = true;
      ROS_INFO("IK succeeded");
    }else{
      ik_bool_msg.data = false;
      ROS_INFO("IK failed");
    }
    ik_bool_pub.publish(ik_bool_msg);
  }

  
  if (enable_ik)
    robot->sendModelAngles(100, aero::ikrange::arm_lifter);
  else if (enable_right_shoulder || enable_right_elbow || enable_right_wrist)
    robot->sendModelAngles(100, aero::ikrange::arm);
...
(const sensor_msgs::Joy::ConstPtr &joy_msg) {
  if (joy_msg->buttons[switch_basic_mode]){
    mode = aero::teleop::BASIC_MODE;
    ROS_INFO("%s:%d", "BASIC_MODE", mode);

  }else if ((mode == aero::teleop::BASIC_MODE) && joy_msg->buttons[switch_mode]){
    mode = aero::teleop::JOINT_MODE;
    ROS_INFO("switched JOINT_MODE");

  }else if ((mode == aero::teleop::JOINT_MODE) && joy_msg->buttons[switch_mode]){
    ROS_INFO("switched IK_MODE");
    mode = aero::teleop::IK_MODE;
  }
  ...
  if (mode == aero::teleop::JOINT_MODE) {
    ...
    enable_ik = false;
    jointMode(joy_msg);
  } else if(mode == aero::teleop::IK_MODE){
    enable_right_shoulder = false;
    enable_right_elbow = false;
    enable_right_wrist = false;
    ikMode(joy_msg);
...
void aero::teleop::ps_teleop::ikMode
(const sensor_msgs::Joy::ConstPtr &joy_msg) {
...
// handle twist
  ...
  //ik
  if (joy_msg->buttons[ik_map[aero::teleop::RIGHT_FLAG]]) {
    enable_ik = true;
    
    pos_dp.x() = joy_msg->axes[ik_map[aero::teleop::X]]*scale_ik_map[aero::teleop::X];
    ROS_DEBUG("pos_dp_x:%f",pos_dp.x() );
    pos_dp.y() = joy_msg->axes[ik_map[aero::teleop::Y]]*scale_ik_map[aero::teleop::Y];
    ROS_DEBUG("pos_dp_y:%f",pos_dp.y() );
    pos_dp.z() = joy_msg->axes[ik_map[aero::teleop::Z]]*scale_ik_map[aero::teleop::Z];
    ROS_DEBUG("pos_dp_z:%f",pos_dp.z() );
